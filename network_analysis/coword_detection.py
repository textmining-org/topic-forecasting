...
def _convert_time_map_by_month_(time_map:dict):
    # 'YYYY-MM-DD' --> 'YYYY_MM'
    by_month_time_map = {_k:f"{int(_v.split('-')[0]):04d}_{int(_v.split('-')[1]):02d}" for _k, _v in time_map.items()}
...
def get_wordcount_dict(preprocessed_data_dict:dict, #{IDX:[WORD]}
    ...
    if timeline_normalization: # frequency rate
        for _t in times:
            ...
            for _w in list(_wc_d):
                by_month_word_count[_t][_w] = by_month_word_count[_t][_w]/_timeline_sum
...
def make_coword_dict(preprocessed_data_dict:dict, #{IDX:[WORD]}
    ...
    if input_at_document_level:
        ...
        for _k, sents in preprocessed_data_dict.items():
            ...
            if document_normalization and not sentence_normalization:
                ...
                for _sent in sents:
                    curr_idx_words.extend(_sent)
                curr_idx_words_c_d = {i:curr_idx_words.count(i) for i in set(curr_idx_words)}
    ...
    if timeline_normalization: # convert to cooccurrence count to rate
        for _t in times:
            ...
            for _cocr_pair in list(_cocr_c_d):
                by_month_coword_dict[_t][_cocr_pair] = by_month_coword_dict[_t][_cocr_pair]/_timeline_sum
...
def parse_preprocessed_data(data_file:str,
    ...
    results = (word_list, whole_word_count, by_month_word_count, whole_coword_map, by_month_coword_map)
    if output_file:
