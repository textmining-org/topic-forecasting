...
if __name__ == "__main__":
    ...
    true_ys, fcst_ys = load_fcst_y(fcst_val_save_path, media, args.model, node_feature_type)
    ...
    for topic_num in range(true_ys.shape[0]):
        ...
        plt.title(f'{media.capitalize()} Topic-{topic_num} Forecasting based on {str(args.model).upper()}')
        plt.legend(loc='upper left')
        plt.ylabel("keywords frequency")
        plt.xlabel("timeline")
        # plt.tight_layout()

        filename = f'{media}_{args.model}_{node_feature_type}_{topic_num}.png'
